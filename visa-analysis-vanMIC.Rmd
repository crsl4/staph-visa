...
- From nonsynonimous data:
  - only interaction of WP_000871607.1 and WP_001060462.1 was barely not significant (pvalue=0.08, negative coefficient)
  - There was no evidence that interaction effects were different by background (CC)
  - WP_000871607.1 (pvalue=0.0075) and WP_001788197.1 (pvalue=0.0032) are significantly associated with van_mic (positive coefficient)
- From all data:
  - WP_001106057.1 (pvalue=0.014) is significantly associated with van_mic (negative coefficient)
  - no interaction was significant. Closer to significance: `WP_001060462.1*WP_001106057.1` with pvalue=0.1 (positive coefficient)
  - There was no evidence that interaction effects were different by background (CC)
  - Significant interaction between WP_001788197.1 and CC (pvalue=0.029) with positive coefficient(WP_001788197.1:ccNRS70)=2.1845
...
interactionPlot = function(df,i,j){
  p <- ggplot(df,aes(x=df[,i],y=van_mic, color=as.factor(df[,j])))+
...
- WP_000871607.1 and WP_000918667.1: not significant (pvalue=0.62)
- WP_000871607.1 and WP_00101713.1: not significant (pvalue=0.11)
- WP_001788197.1 and WP_001017131.1: not significant (pvalue=0.20)
- WP_001788197.1 and WP_000149064.1: not significant (pvalue=0.20)
- WP_000871607.1 and WP_001060462.1: barely not significant (pvalue=0.08)
- WP_001060462.1 and WP_001788197.1: not significant (pvalue=0.36)
- WP_001017131.1 and WP_001060462.1: not significant (pvalue=0.91)
...
mod1 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod1)
```
...
```{r}
mod2 = lmer(van_mic ~ r + WP_000871607.1*WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod2)
...
```{r}
mod3 = lmer(van_mic ~ r + WP_000871607.1*WP_001017131.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod3)

anova(mod1,mod3)

mod3.2 = lmer(van_mic ~ r + WP_000871607.1*WP_001017131.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_000149064.1 + (1 + WP_000871607.1*WP_001017131.1| cc), dat10.c)
summary(mod3.2)
...
```{r}
mod4 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1*WP_001017131.1 + WP_001060462.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod4)

anova(mod1,mod4)

mod4.2 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1*WP_001017131.1 + WP_001060462.1 + WP_000149064.1 + (1+WP_001788197.1*WP_001017131.1 | cc), dat10.c)
summary(mod4.2)
...
```{r}
mod5 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1*WP_000149064.1 + WP_001060462.1 + WP_001017131.1 + (1 | cc), dat10.c)
summary(mod5)
...
```{r}
mod6 = lmer(van_mic ~ r + WP_000871607.1*WP_001060462.1 + WP_000918667.1 + WP_001788197.1 + WP_001017131.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod6)
...
```{r}
mod7 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1*WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod7)
...
```{r}
mod8 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1*WP_001017131.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod8)
...
```{r}
m1 = lm(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + cc, dat10.c)
summary(m1)

## from mod6, the only interaction close to significance:
m6 = lm(van_mic ~ r + WP_000871607.1*WP_001060462.1*cc + WP_000918667.1 + WP_001788197.1 + WP_001017131.1 + WP_000149064.1, dat10.c)
summary(m6)
...
- NRS384: WP_000149064.1 and WP_001106057.1
...
- WP_001060462.1 and WP_001106057.1 (pvalue=0.1)
...
mod1 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1 + (1 | cc), dat10.c)
summary(mod1)
```

Then, we fit the models with interactions one by one and compare to the model without interactions with ANOVA. We fit two different interaction models: one with scalar random effects (interactions do not depend on CC), and one with vector random effects (interactions depend on CC). 
```{r}
mod2 = lmer(van_mic ~ r + WP_000871607.1*WP_001106057.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod2)
...
```{r}
mod3 = lmer(van_mic ~ r + WP_000871607.1 + WP_001017131.1 + WP_000918667.1*WP_001106057.1 + WP_001788197.1 + WP_001060462.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod3)
...
```{r}
mod4 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1*WP_001106057.1 + WP_001017131.1 + WP_001060462.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod4)
...
```{r}
mod5 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_000149064.1 + WP_001060462.1*WP_001106057.1 + WP_001017131.1 + (1 | cc), dat10.c)
summary(mod5)
...
```{r}
mod6 = lmer(van_mic ~ r + WP_000871607.1 + WP_001060462.1 + WP_000918667.1 + WP_001788197.1 + WP_001017131.1*WP_001106057.1 + WP_000149064.1 + (1 | cc), dat10.c)
summary(mod6)
...
```{r}
mod7 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1*WP_001106057.1 + (1 | cc), dat10.c)
summary(mod7)
...
mod1.1 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1 + (1 + WP_000871607.1 | cc), dat10.c)

anova(mod1,mod1.1)

mod1.2 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1 + (1 + WP_001788197.1 | cc), dat10.c)

anova(mod1,mod1.2)

mod1.3 = lmer(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1 + (1 + WP_001106057.1 | cc), dat10.c)
...
```{r}
mod1 = lm(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1 + cc, dat10.c)
summary(mod1)

mod1.1 = lm(van_mic ~ r + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1 + WP_000871607.1*cc, dat10.c)

anova(mod1,mod1.1)

mod1.2 = lm(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1 + WP_001788197.1*cc, dat10.c)
summary(mod1.2)
anova(mod1,mod1.2)

mod1.3 = lm(van_mic ~ r + WP_000871607.1 + WP_000918667.1 + WP_001788197.1 + WP_001060462.1 + WP_001017131.1 + WP_000149064.1 + WP_001106057.1*cc, dat10.c)
summary(mod1.3)
